...
import { EmailAddress, IUserRequest } from '../models/ICredentials';
...
const ResetPassword = () => {
  ...
  const { register, handleSubmit, formState: form, getFieldState } = useForm<EmailAddress>({ resolver: zodResolver(schema), mode: 'onTouched' });
  const [resetPassword, { data, error, isLoading, isSuccess }] = userAPI.useResetPasswordMutation();

  const isFieldValid = (fieldName: keyof EmailAddress): boolean => (getFieldState(fieldName, form).isTouched && !getFieldState(fieldName, form).invalid);

  const handleResetPassword = async (credentials: IUserRequest) => await resetPassword(credentials);
              ...
              <form onSubmit={handleSubmit(handleResetPassword)} className="needs-validation" noValidate>
                ...
                <div className="col mt-3">
                  <button disabled={form.isSubmitting || isLoading} className="btn btn-primary btn-block" type="submit" >
                    ...
                    <span role="status">{(form.isSubmitting || isLoading) ? 'Loading...' : 'Reset'}</span>
                  </button>
                </div>
              </form>
              <hr className="my-3" />
