...
const Authentication = () => {
  ...
  const toggleMfa = async () => {
    user.data.user.is_mfa ? await disableMfa() : await enableMfa();
  };
  ...
  return (
    <>
      {isLoading && <Loader /> }
      {isSuccess && <>
        <h4 className="mb-3">Authentication(MFA) <span className={`badge pill text-light text-bg-${user?.data.user.is_mfa ? 'success' : 'warning'} fs-6`}>{user?.data.user.is_mfa ? 'Enabled' : 'Disabled'}</span></h4>
        <hr />
        <div className="row g-3">
          <div className="col-12 mb-2">
            ...
            <button onClick={toggleMfa} disabled={isLoading || disableLoading || enableLoading} className={`btn border btn-${user?.data.user.is_mfa ? 'light' : 'primary'} mt-2`} type="button">{user?.data.user.is_mfa ? 'Disable' : 'Enable'} Two-Factor Authentication
              {(isLoading || disableLoading || enableLoading) && <div className={`spinner-border text-${user?.data.user.is_mfa ? 'primary' : 'light'}`} role="status" style={{ height: '20px', width: '20px', marginLeft: '10px' }}></div>}
            </button>
            {(user?.data.user.is_mfa) && <div className="accordion mt-3 mb-3" id="accordionExample">
