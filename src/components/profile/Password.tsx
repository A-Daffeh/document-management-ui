...
const schema = z.object({
  ...
  confirmNewPassword: z.string().min(3, 'Confirm password is required'),
...
const Password = () => {
  const { register, handleSubmit, reset, formState: form, getFieldState } = useForm<UpdatePassword>({ resolver: zodResolver(schema), mode: 'onTouched', defaultValues: { password: '', newPassword: '', confirmNewPassword: '' } });
  const { data: user, error, isLoading, isSuccess, refetch } = userAPI.useFetchUserQuery();
  const [updatePassword, { data: qrData, isLoading: updateLoading, isSuccess: updateSuccess }] = userAPI.useUpdatePasswordMutation();
